...
#define int ll
ll lazy[4*MAXN+1];
void buildTree(vl&a,vl&tree,int s,int e,int index){
    if(s==e){
        tree[index]=a[s];
        return;
    }
...
void updateLazy(vl&tree,int s,int e,int l,int r,int inc,int index){
    if(lazy[index]!=0){
    ...
    int mid=(s+e)/2;
    updateLazy(tree,s,mid,l,r,inc,2*index);
    updateLazy(tree,mid+1,e,l,r,inc,2*index+1);
    tree[index]=tree[2*index]+tree[2*index+1];
}
ll queryLazy(vl&tree,int s,int e,int l,int r,int index){
    if(lazy[index]!=0){
        tree[index]+=lazy[index]*(e-s+1);
        if(s!=e){
            lazy[2*index]+=lazy[index];
    ...
    if(s>=l&&e<=r)
        return tree[index];
    int mid=(s+e)/2;
    return queryLazy(tree,s,mid,l,r,2*index)+queryLazy(tree,mid+1,e,l,r,2*index+1);
}
void solve()
{
    int n,k,q;
    ...
    for(int i=1;i<n;i++){
        f[i]=f[i-1]+a[i]-a[(n-k+i)%n];
    }
    vl tree(4*n+1);
    buildTree(f,tree,0,n-1,1);
    FOR(i,q){
        int type;
        cin>>type;
        if(type==1){
            int x,y;
            cin>>x>>y;
            int l=x-1,r=(x-1+k-1+n)%n;
            int inc=y-a[l];
            a[l]=y;
            if(l>r){
                updateLazy(tree,0,n-1,l,n-1,inc,1);
                updateLazy(tree,0,n-1,0,r,inc,1);
            }
            else{
                updateLazy(tree,0,n-1,l,r,inc,1);
            }
        }
        else{
            int l,r;
            cin>>l>>r;
            cout<<queryLazy(tree,0,n-1,(l-1+n)%n,(r-1+n)%n,1)<<endl;
        }
    }
}