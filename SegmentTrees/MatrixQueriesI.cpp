...
vector<vl> matrixMultiply(vector<vl> &a, vector<vl> &b)
{
    ...
    FOR(i, a.size())
    {
        FOR(j, b[0].size())
        {
            FOR(k, a[0].size())
            {
                res[i][j] = (res[i][j]+(a[i][k] * b[k][j])%R)%R;
            }
        }
    }
    return res;
...
/*Your functions here*/
void buildTree(vector<vector<vi>>&tree,vector<vector<vi>>&arr,int s,int e,int index){
    if(s==e){
        tree[index]=arr[s];
        return;
    }
...
vector<vi>temp(2,vi(2,0));
void query(vector<vector<vi>>&tree,vector<vi>&arr,int s,int e,int l,int r,int index){
    if(s>r || e<l){
        return;
    }
    if(s>=l && e<=r){
        FOR(i,2){
            FOR(j,2){
                temp[i][j]=0;
                FOR(k,2){
                    temp[i][j]=(temp[i][j]+(arr[i][k]*tree[index][k][j])%R)%R;
                }
            }
        }
        arr=temp;
    ...
    int mid=(s+e)/2;
    query(tree,arr,s,mid,l,r,2*index);
    query(tree,arr,mid+1,e,l,r,2*index+1);
}
void solve()
{
    int n,q;
    cin>>R>>n>>q;
    vector<vector<vi>> array(n,vector<vi>(2,vi(2)));
    FOR(i,n)
    {
        FOR(j,2)
        {
    ...
    vector<vector<vi>> tree(4*n+1,vector<vi>(2,vi(2)));
    buildTree(tree,array,0,n-1,1);
    FOR(i,q)
    {
        int l,r;
        cin>>l>>r;
