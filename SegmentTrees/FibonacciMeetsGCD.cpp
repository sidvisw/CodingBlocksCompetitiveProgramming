...
ll fibonacci_helper(ll n)
{
    ...
    if (n & 1)
    {
        return fib[n] = ((fibonacci_helper(n / 2) * fibonacci_helper(n / 2 + 1))%MOD + (fibonacci_helper(n / 2) * fibonacci_helper(n / 2 - 1))%MOD)%MOD;
    }
    else
    {
        return fib[n] = ((fibonacci_helper(n / 2) * fibonacci_helper(n / 2))%MOD + (fibonacci_helper(n / 2 - 1) * fibonacci_helper(n / 2 - 1))%MOD)%MOD;
    }
}
ll fibonacci(ll n)
{
...
/*Your functions here*/
void buildTree(vi&tree,vi&a,int s,int e,int index){
    if(s==e){
        tree[index]=a[s];
        return;
    }
...
int query(vi&tree,int s,int e,int l,int r,int index){
    ...
    int mid=(s+e)/2;
    return __gcd(query(tree,s,mid,l,r,2*index),query(tree,mid+1,e,l,r,2*index+1));
}
void solve()
{
    int n,q;
    ...
    while(q--){
        ...
        l--;
        r--;
        cout<<fibonacci(query(tree,0,n-1,l,r,1))<<endl;
    }
}