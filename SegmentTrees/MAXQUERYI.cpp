...
/*Your functions here*/
void buildTree(vi&a,vector<vi>&tree,int s,int e,int index){
    if(s==e){
    ...
    while(i<tree[2*index].size()&&j<tree[2*index+1].size()){
        if(tree[2*index][i]<tree[2*index+1][j]){
            tree[index].push_back(tree[2*index][i]);
            i++;
        }
        else{
            tree[index].push_back(tree[2*index+1][j]);
            j++;
        }
    }
...
int query(vector<vi>&tree,int s,int e,int i,int j,int k,int index){
    if(i>e||j<s){
        return 0;
    }
    if(s>=i&&e<=j){
        return tree[index].end()-lower_bound(all(tree[index]),k);
    }
    int mid=(s+e)/2;
    return query(tree,s,mid,i,j,k,2*index)+query(tree,mid+1,e,i,j,k,2*index+1);
}
void solve()
{
    int n;
    ...
    while(q--){
        ...
        cin>>i>>j>>k;
        i--;j--;
        lastans=query(tree,0,n-1,i,j,k,1);
        cout<<lastans<<endl;
    }
}