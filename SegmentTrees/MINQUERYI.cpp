...
/*Your functions here*/
void build(int tree[],int a[],int s,int e,int index){
    if(s==e){
        tree[index]=a[s];
        return;
    }
    int mid=(s+e)/2;
    build(tree,a,s,mid,2*index);
    build(tree,a,mid+1,e,2*index+1);
    tree[index]=min(tree[2*index],tree[2*index+1]);
}
void updateNode(int tree[],int s,int e,int x,int y,int index){
    if(x<s||x>e){
        return;
    }
    if(s==e){
    ...
    int mid=(s+e)/2;
    updateNode(tree,s,mid,x,y,2*index);
    updateNode(tree,mid+1,e,x,y,2*index+1);
    tree[index]=min(tree[2*index],tree[2*index+1]);
}
int query(int tree[],int s,int e,int l,int r,int index){
    ...
    int mid=(s+e)/2;
    int q1=query(tree,s,mid,l,r,2*index);
    int q2=query(tree,mid+1,e,l,r,2*index+1);
    return min(q1,q2);
}
void solve()
{
