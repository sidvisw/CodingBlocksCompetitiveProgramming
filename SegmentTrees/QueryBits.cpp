...
/*Your functions here*/
int lazy[4*MAXN+1];
void updateRange0(vi&tree,int s,int e,int l,int r,int index){
    if(lazy[index]!=0){
        if(lazy[index]==1){
            tree[index]=(fastExponentiation(2,e-s+1)-1+MOD)%MOD;
        }
        else{
            tree[index]=0;
        }
    ...
    int mid=(s+e)/2;
    updateRange0(tree,s,mid,l,r,2*index);
    updateRange0(tree,mid+1,e,l,r,2*index+1);
    tree[index]=((tree[2*index]*fastExponentiation(2,e-mid))%MOD+tree[2*index+1])%MOD;
}
void updateNode1(vi&tree,int s,int e,int l,int r,int index){
    if(lazy[index]!=0){
        if(lazy[index]==1){
            tree[index]=(fastExponentiation(2,e-s+1)-1+MOD)%MOD;
        }
    ...
    int mid=(s+e)/2;
    updateNode1(tree,s,mid,l,r,2*index);
    updateNode1(tree,mid+1,e,l,r,2*index+1);
    tree[index]=((tree[2*index]*fastExponentiation(2,e-mid))%MOD+tree[2*index+1])%MOD;
}
int query(vi&tree,int s,int e,int l,int r,int index){
    if(lazy[index]!=0){
        if(lazy[index]==1){
            tree[index]=(fastExponentiation(2,e-s+1)-1+MOD)%MOD;
        }
    ...
    if(s>r||e<l)
        return 0;
    if(s>=l&&e<=r){
        return (tree[index]*fastExponentiation(2,r-e))%MOD;
    }
    int mid=(s+e)/2;
...
void solve()
{
    ...
    FOR(i,q){
        ...
        if(type==0){
            int l,r;
            cin>>l>>r;
            updateRange0(tree,0,n-1,l,r,1);
        }
        else if(type==1){
            int l,r;
            cin>>l>>r;
            updateNode1(tree,0,n-1,l,r,1);
        }
        else{
            int l,r;
            cin>>l>>r;
            cout<<query(tree,0,n-1,l,r,1)%MOD<<endl;
        }
    }
}