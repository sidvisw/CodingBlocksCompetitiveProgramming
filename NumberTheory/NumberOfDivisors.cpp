...
void factorize(ll n){
    ...
    for(ll i=2;i*i<=n;i++){
        if(n%i==0){
            while(n%i==0){
                n/=i;
                PrimeFactorCount[i]=(PrimeFactorCount[i]+1)%MOD;
            }
        }
    }
    if(n>1){
        PrimeFactorCount[n]=(PrimeFactorCount[n]+1)%MOD;
    }
}
void solve()
{
    ...
    ll prod=1;
    for(auto &p:PrimeFactorCount){
        prod=(prod*(p.ss+1))%MOD;
    }
    cout<<prod;
}