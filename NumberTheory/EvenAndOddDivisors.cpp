...
void solve()
{
    int n;
    cin>>n;
    vii factors=factorize(n);
    ll sumTotalFactors=1;
    ll sumOddFactors=1;
    for(auto p:factors){
        sumTotalFactors*=(pow(p.first,p.second+1)-1)/(p.first-1);
    }
    for(auto p:factors){
        if(p.first%2==1){
            sumOddFactors*=(pow(p.first,p.second+1)-1)/(p.first-1);
        }
    }
    cout<<sumTotalFactors-2*sumOddFactors;
}