...
public:
    int x1, y1, x2, y2;
    int t;
    bool operator<(const Pipeline &p) const
    {
        return min(manDist(x1, y1, sx, sy), manDist(x2, y2, sx, sy)) < min(manDist(p.x1, p.y1, sx, sy), manDist(p.x2, p.y2, sx, sy));
    }
};
/*-------------------------IMPORTANT #DEFINES---------------------------*/
#define endl '\n'
...
vector<vector<Pipeline>> generateSubsets(vector<Pipeline> v)
{
    if (v.size() == 0)
    {
        vector<vector<Pipeline>> res;
        res.pb({});
        return res;
    }
    Pipeline last = v.back();
    v.pop_back();
    vector<vector<Pipeline>> res = generateSubsets(v);
    ll n = res.size();
    FOR(i, n)
    {
        res.pb(res[i]);
...
void solve()
{
    ...
    vector<vector<Pipeline>> subsets = generateSubsets(Pipes);
    for(auto &subset : subsets)
    {
        sort(all(subset));
    }
    int minTime = manDist(sx, sy, ex, ey);
    for (auto &pipes : subsets)
    {
        bool cond = true;
        while (cond)
        {
            ...
            for (auto &p : pipes)
            {
                if (manDist(Sx, Sy, p.x1, p.y1) > manDist(Sx, Sy, p.x2, p.y2))
                {
                    time += manDist(Sx, Sy, p.x2, p.y2);
                    time += p.t;
                    Sx = p.x1;
                    Sy = p.y1;
                }
                ...
                {
                    time += manDist(Sx, Sy, p.x1, p.y1);
                    time += p.t;
                    Sx = p.x2;
                    Sy = p.y2;
                }
            }
            time += manDist(Sx, Sy, ex, ey);
            minTime = min(minTime, time);
            cond=next_permutation(all(pipes));
        }
    }
    cout << minTime;
}