...
/*Your functions here*/
void PhoneKeypadProblem(string s, string &ans)
{
    ...
    if (s.length() == 0)
    {
        ...
        for (int i = 0; i <= 10; i++)
        {
            if (searchIn[i].find(ans) != string::npos)
            {
    ...
    FOR(i, keypad[s[0] - '0'].length())
    {
        ans += keypad[s[0] - '0'][i];
        PhoneKeypadProblem(s.substr(1), ans);
        ans.pop_back();
    }
}
void solve()
