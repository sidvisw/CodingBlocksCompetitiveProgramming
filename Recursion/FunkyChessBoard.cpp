...
/*Your functions here*/
vii moves={{2,1},{2,-1},{-2,1},{-2,-1},{1,2},{1,-2},{-1,2},{-1,-2}};
vii calculateMoves(vector<vi>&board,int x,int y){
    vii newPos;
    for(auto move:moves){
        int newX=x+move.ff;
        int newY=y+move.ss;
        if(newX>=0 && newX<board.size() && newY>=0 && newY<board[0].size()){
            if(board[newX][newY]==1){
                newPos.push_back({newX,newY});
            }
        }
    }
    return newPos;
}
void maxOccupancyOfKnight(vector<vi>&board,int x,int y,ll &count){
    vii newPos=calculateMoves(board,x,y);
    if(newPos.empty()){
        ll cnt=0;
        FOR(i,board.size()){
            FOR(j,board[0].size()){
    ...
    for(auto pos:newPos){
            board[pos.ff][pos.ss]=-1;
            maxOccupancyOfKnight(board,pos.ff,pos.ss,count);
            board[pos.ff][pos.ss]=1;
    }
}
void solve()
{
    ...
    board[i][j]=-1;
    maxOccupancyOfKnight(board,i,j,count);
    cout<<count1-count;
}