...
/*Your functions here*/
int **dp;
int getScore(vi &a, int low, int high)
{
    if (dp[low][high] != -1)
        return dp[low][high];
    if (low > high)
    ...
    if ((high - low + 1) & 1)
    {
        return dp[low][high] = min(getScore(a, low + 1, high), getScore(a, low, high - 1));
    }
    else
    {
        return dp[low][high] = max(a[low] + getScore(a, low + 1, high), a[high] + getScore(a, low, high - 1));
    }
}
void solve()
{
