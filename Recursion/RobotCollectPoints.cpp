...
/*Your functions here*/
int getScore(vector<vii>&mat,int pos,int h){
    if(h==0){
        return 0;
    }
    int pos1=pos-1;
    ...
    int score3=0;
    if(pos1>=0){
        score1=mat[h-1][pos1].ff+getScore(mat,pos1,h-1);
    }
    if(pos2<mat[h].size()){
        score2=mat[h-1][pos2].ff+getScore(mat,pos2,h-1);
    }
    score3=mat[h-1][pos].ff+getScore(mat,pos,h-1);
...
void solve()
{
    ...
    FOR(k,h-5+1){
        ...
        score=max(score,getScore(mat,2,h));
        FOR(i,5){
            FOR(j,5){
                if(mat[i+k][j].ss==1){
                    mat[i+k][j].ff=-1;
