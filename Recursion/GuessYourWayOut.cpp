...
ll count(int N,int H){
    ...
    int ancestor=ceil(N/2.0);
    int ancestorofancestor=ceil(ancestor/2.0);
    if(ancestorofancestor*2-1==ancestor){
        if(ancestor*2-1==N){
            return count(ancestor,H-1)+pow(2,h-H+1);
        }
        else{
            return 1+count(ancestor,H-1);
        }
    }
    else{
        if(ancestor*2-1==N){
