...
/*Your functions here*/
void floodFill(vector<string>&mat,int i,int j,char x,char c){
	if(i<0 || i>=mat.size() || j<0 || j>=mat[0].size() || mat[i][j]!=x)
		return;
	mat[i][j]=c;
	floodFill(mat,i+1,j,x,c);
	floodFill(mat,i-1,j,x,c);
	floodFill(mat,i,j+1,x,c);
	floodFill(mat,i,j-1,x,c);
}
void solve()
{
