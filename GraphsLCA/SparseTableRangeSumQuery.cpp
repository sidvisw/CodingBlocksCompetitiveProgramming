...
/*Your functions here*/
vi LOG2;
void buildTable(vi&a,vector<vi>&table){
	table.resize(a.size(),vi(ceil(log2(a.size()))+1));
	LOG2.clear();
	LOG2.resize(a.size()+1);
	FORr(i,2,a.size()+1){
		LOG2[i]=LOG2[i/2]+1;
	}
	FOR(i,a.size()){
	...
	FORe(j,table[0].size()-1){
		for(int i=0;i+(1<<j)<=a.size();i++){
			table[i][j]=(table[i][j-1]+table[i+(1<<(j-1))][j-1]);
		}
	}
}
int query(int l,int r,vector<vi>&table){
	int sum=0;
	int len=r-l+1;
	while(len){
		int pw=LOG2[len];
