...
int n;
ldd dp[305][305][305];
ldd expectedMoves(int x,int y,int z){
	if(x==0&&y==0&&z==0){
		return 0;
	}
	if(dp[x][y][z]>=-0.9){
	...
	return dp[x][y][z]=(n+x*((x==0)?0.0:expectedMoves(x-1,y,z))+y*((y==0)?0.0:expectedMoves(x+1,y-1,z))+z*((z==0)?0.0:expectedMoves(x,y+1,z-1)))/(x+y+z);
}
void solve()
{
	cin>>n;
	...
	cout<<setprecision(__PRECISION)<<fixed<<expectedMoves(m[1],m[2],m[3]);
}