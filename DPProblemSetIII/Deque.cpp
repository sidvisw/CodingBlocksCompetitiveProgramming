...
int X=0,Y=0;
int dp[3005][3005];
int calculateX(vi &a,int i,int j){
	if(i>j){
		return 0;
	}
	if(dp[i][j]!=-1){
	...
	int s1=calculateX(a,i+2,j);
	int s2=calculateX(a,i+1,j-1);
	int score1=min(s1,s2);
	s1=calculateX(a,i,j-2);
	int score2=min(s1,s2);
	return dp[i][j]=max(a[i]+score1,a[j]+score2);
}
void solve()
{
	int n;
	...
	int X=calculateX(a,0,n-1);
	int Y=accumulate(all(a),0ll)-X;
	cout<<X-Y;
}