...
/*Your functions here*/
int lazy[4*MAXN+1];
void buildTree(vi&a,vl&tree,int s,int e,int index){
    if(s==e){
        tree[index]=a[s];
        return;
    }
...
void updateRangeLazy(vl&tree,int s,int e,int l,int r,int inc,int index){
    if(lazy[index]!=0){
...
ll queryLazy(vl&tree,int s,int e,int l,int r,int index){
    ...
    int mid=(s+e)/2;
    ll left=queryLazy(tree,s,mid,l,r,2*index);
    ll right=queryLazy(tree,mid+1,e,l,r,2*index+1);
    return left+right;
}
void solve()
{
