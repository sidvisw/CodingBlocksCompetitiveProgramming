...
ll mod_Exponentiation(ll a, ll b,ll m)
{
    ll res = 1;
    while (b > 0)
    {
        if (b & 1)
        {
            res = mod_multiply(res,a,m);
        }
        b >>= 1;
        a = mod_multiply(a,a,m);
    }
    return res;
}
...
bool miller_rabin(int n){
    ...
    for(auto a:A){
        if(a>n-2)break;
        int r=mod_Exponentiation(a,d,n);
        if(r==1)continue;
        bool prime=false;
        FOR(i,s){
            int p=mod_Exponentiation(2,i,n);
            int ar=mod_Exponentiation(r,p,n);
            if(ar==n-1){
                prime=true;
                break;
            }
