...
void solve()
{
	...
	vii circles(n);
	FOR(i,n){
		cin>>circles[i].ff>>circles[i].ss;
	}
	vii startEnd(n);
	FOR(i,n){
		startEnd[i].ff=circles[i].ff-circles[i].ss;
		startEnd[i].ss=circles[i].ff+circles[i].ss;
	}
	sort(all(startEnd),comp);
	ll maxSelectableCircle=0;
	int prevEnd=-inf;
	FOR(i,n){
		if(startEnd[i].ff>=prevEnd){
			maxSelectableCircle++;
			prevEnd=startEnd[i].ss;
		}
	}
	cout<<n-maxSelectableCircle;
}