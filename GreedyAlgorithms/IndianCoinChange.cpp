...
int coinChange(int money){
	vi denominations{1,2,5,10,20,50,100,200,500,2000};
	int cnt=0;
	while(money){
		int deno=*(upper_bound(denominations.begin(),denominations.end(),money)-1);
		int count=money/deno;
		cnt+=count;
		money-=count*deno;
	}
	return cnt;
...
void solve()
{
	int money;
	cin>>money;
	cout<<"Minimum Coins needed is "<<coinChange(money);
}