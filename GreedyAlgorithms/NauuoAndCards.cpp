...
bool isPossible(multiset<int> &nauuo,vi &pile,int moves){
	set<int>tillNow;
	FOR(i,moves){
		tillNow.insert(pile[i]);
	}
	int idx=moves;
	FORe(i,pile.size()){
		if(tillNow.find(i)==tillNow.end()&&nauuo.find(i)==nauuo.end()){
			return false;
		}
		if(idx<pile.size()){
			tillNow.insert(pile[idx++]);
		}
	}
	return true;
}
void BinarySearch(multiset<int>&nauuo,vi &pile,int begin,int end,int &idx)
{
	if (begin > end)
	{
		return;
	...
	ll mid = (begin + end) / 2;
	if (isPossible(nauuo,pile,mid))
	{
		idx = mid;
		BinarySearch(nauuo,pile,begin,mid-1,idx);
	}
	else
	{
		BinarySearch(nauuo,pile,mid+1,end,idx);
	}
}
...
void solve()
{
	...
	if(!last){
		...
		set<int>tillNow;
		for(int i=num;i<=n;i++){
			if(nauuo.find(i)==nauuo.end()&&tillNow.find(i)==tillNow.end()){
				cond=true;
				break;
			}
			tillNow.insert(pile[idx++]);
	...
	int ans=n;
	BinarySearch(nauuo,pile,0,n,ans);
	cout<<ans+n;
}