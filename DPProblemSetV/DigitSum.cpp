...
/*Your functions here*/
int dp[10005][105][2];
int possibleNums(string&k,int i,int mod,int constraint,int d){
	if(i==k.size()){
		if(mod==0){
			return 1;
		}
	...
	if(dp[i][mod][constraint]!=-1){
		return dp[i][mod][constraint];
	}
	int ans=0;
	if(constraint){
		for(int j=0;j<k[i]-'0';j++){
			ans=(ans+possibleNums(k,i+1,(mod-(j%d)+d)%d,0,d))%MOD;
		}
		ans=(ans+possibleNums(k,i+1,(mod-((k[i]-'0')%d)+d)%d,1,d))%MOD;
	}
	else{
		for(int j=0;j<=9;j++){
			ans=(ans+possibleNums(k,i+1,(mod-(j%d)+d)%d,0,d))%MOD;
	...
	return dp[i][mod][constraint]=ans;
}
void solve()
{
	string k;
	...
	cout<<(possibleNums(k,0,0,1,d)-1+MOD)%MOD;
}