...
/*Your functions here*/
int dp[3005][3005];
int ways(string &s,int idx,int g){
	if(idx==s.size()){
		return 1;
	}
	if(dp[idx][g]!=-1){
	...
	if(s[idx]=='<'){
		...
		return dp[idx][g]=(ways(s,idx+1,g-1)+ways(s,idx,g-1))%MOD;
	}
	else{
		if(g==0){
			int total=s.size()-idx;
			int ans=0;
			for(int i=0;i<total;i++){
				ans=(ans+ways(s,idx+1,i))%MOD;
			}
			return dp[idx][g]=ans;
		}
		return dp[idx][g]=(ways(s,idx,g-1)-ways(s,idx+1,g-1)+MOD)%MOD;
	}
}
void solve()
{
