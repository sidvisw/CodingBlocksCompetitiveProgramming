...
public:
	...
	int dfs(ll src, int par)
	{
		...
		for (auto v : adjList[src])
		{
			if (v.ff!=par)
			{
				ans=(ans*(dfs(v.ff,src)+1)%MOD)%MOD;
			}
		}
		return dp1[src]=ans;
	}
	void dfs2(int src,int parVal,int par){
		dp2[src]=(dp1[src]*((parVal+1)%MOD))%MOD;
		vector<int>prefix,suffix;
		for(auto v:adjList[src]){
			if(v.ff!=par){
				prefix.pb((dp1[v.ff]+1)%MOD);
				suffix.pb((dp1[v.ff]+1)%MOD);
			}
		}
		...
		for(auto v:adjList[src]){
			if(v.ff!=par){
				int ways=(1+parVal)%MOD;
				if(cno>0){
					ways=(ways*prefix[cno-1])%MOD;
				}
				if(cno<suffix.size()-1){
					ways=(ways*suffix[cno+1])%MOD;
				}
				dfs2(v.ff,ways,src);
				cno++;
			}
