...
void updateNode(vi&tree,int s,int e,int p,int val,int index){
    if(p<s||p>e){
        return;
    }
    if(s==e){
    ...
    int mid=(s+e)/2;
    updateNode(tree,s,mid,p,val,2*index);
    updateNode(tree,mid+1,e,p,val,2*index+1);
    if(n&1){
        if(((int)(log2(index)))&1){
            tree[index]=tree[2*index]^tree[2*index+1];
        }
