// Terminologies:
// close => the required no. of closing brackets needed are given or not
// open => signifies how many more closing brackets are are made
...
/*Your functions here*/
int dp[1205][1205][2];
int len,openingBracket,closingBracket;
int combinations(int i,int open,int close){
	if(i==len){
		if(close&&open==0){
			return 1;
		}
	...
	int ans=0;
	if(!close){
		if(open>=openingBracket){
			ans=(ans+combinations(i+1,open-openingBracket+closingBracket,1))%MOD;
		}
	}
	if(open){
		ans=(ans+combinations(i+1,open-1,close))%MOD;
	}
	ans=(ans+combinations(i+1,open+1,close))%MOD;
