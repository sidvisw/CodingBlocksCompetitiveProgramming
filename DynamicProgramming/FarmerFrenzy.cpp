...
int maxArea(vector<vi>&mat){
	...
	FOR(i,n){
		...
		stk.push(mp(0,0));
		FOR(j,m){
			if(stk.empty()||stk.top().ff<=mat[i][j]){
				stk.push(mp(mat[i][j],j+1));
			}
			else{
				while(stk.top().ff>mat[i][j]){
					pii p=stk.top();
					stk.pop();
					area=max(area,(p.ff*(j-stk.top().ss)));
				}
				stk.push(mp(mat[i][j],j+1));
			}
		}
		while(stk.size()>1){
			pii p=stk.top();
			stk.pop();
			area=max(area,(p.ff*(m-stk.top().ss)));
		}
	}
	return area;
}
