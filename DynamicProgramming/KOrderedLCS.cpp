...
/*Your functions here*/
int dp[2001][2001][6];
int KOrderLCS(vi&a,vi&b,int i,int j,int k){
	...
	int c1=0,c2=0,c3=0;
	if(a[i]==b[j]){
		c1=KOrderLCS(a,b,i+1,j+1,k)+1;
	}
	if(k>0){
		c2=KOrderLCS(a,b,i+1,j+1,k-1)+1;
	}
	c3=max(KOrderLCS(a,b,i+1,j,k),KOrderLCS(a,b,i,j+1,k));
