...
/*Your functions here*/
ldd dp[105][105][105];
ldd calculateProbabilityRock(int r,int s,int p){
	if(r==0 || s==0){
		return 0;
	}
	if(p==0){
	...
	ldd total=r*p+p*s+s*r;
	ldd ans=0;
	ans+=calculateProbabilityRock(r-1,s,p)*(r*p/total);
	ans+=calculateProbabilityRock(r,s,p-1)*(p*s/total);
	ans+=calculateProbabilityRock(r,s-1,p)*(s*r/total);
	return dp[r][s][p]=ans;
}
ldd calculateProbabilityScissor(int r,int s,int p){
	if(p==0 || s==0){
		return 0;
	}
	if(r==0){
	...
	ldd total=r*p+p*s+s*r;
	ldd ans=0;
	ans+=calculateProbabilityScissor(r-1,s,p)*(r*p/total);
	ans+=calculateProbabilityScissor(r,s,p-1)*(p*s/total);
	ans+=calculateProbabilityScissor(r,s-1,p)*(s*r/total);
	return dp[r][s][p]=ans;
}
ldd calculateProbabilityPaper(int r,int s,int p){
	if(p==0 || r==0){
		return 0;
	}
	if(s==0){
	...
	ldd total=r*p+p*s+s*r;
	ldd ans=0;
	ans+=calculateProbabilityPaper(r-1,s,p)*(r*p/total);
	ans+=calculateProbabilityPaper(r,s,p-1)*(p*s/total);
	ans+=calculateProbabilityPaper(r,s-1,p)*(s*r/total);
	return dp[r][s][p]=ans;
}
void solve()
{
	...
	cout<<setprecision(__PRECISION)<<fixed;
	cout<<calculateProbabilityRock(r,s,p)<<" ";
	FOR(i,105){
		FOR(j,105){
			FOR(k,105){
				dp[i][j][k]=-1;
	...
	cout<<calculateProbabilityScissor(r,s,p)<<" ";
	FOR(i,105){
		FOR(j,105){
			FOR(k,105){
				dp[i][j][k]=-1;
	...
	cout<<calculateProbabilityPaper(r,s,p);
}