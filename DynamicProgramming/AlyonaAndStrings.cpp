...
int SumLength(int i,int j,int k,int cont){
	...
	if(dp[i][j][k][cont]!=-1){
		return dp[i][j][k][cont];
	}
	int ans=-INF;
	ans=max(ans,SumLength(i+1,j,k-cont,0));
	ans=max(ans,SumLength(i,j+1,k-cont,0));
	if(s1[i]==s2[j]){
		ans=max(ans,1+SumLength(i+1,j+1,k,1));
		ans=max(ans,1+SumLength(i+1,j+1,k-1,1));
	}
	return dp[i][j][k][cont]=ans;
}
void solve()
{
	int n,m,k;
