...
/*Your functions here*/
vector<vi>dp;
int calculateProfit(int i,int j,int y,vi&p){
	if(i>j){
		return 0;
	}
	if(dp[i][j]!=-1)return dp[i][j];
	return dp[i][j]=max(p[i]*y+calculateProfit(i+1,j,y+1,p),p[j]*y+calculateProfit(i,j-1,y+1,p));
}
void solve()
{
	int n;
	...
	dp.clear();
	dp.resize(n,vi(n,-1));
	cout<<calculateProfit(0,n-1,1,p);
}