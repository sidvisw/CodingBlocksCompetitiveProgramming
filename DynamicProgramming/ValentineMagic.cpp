...
vi gurls;
int dp[5005][5005];
int calculateMinCostOfPairing(int i,int j){
	if(i==0){
		return 0;
	}
	if(i>j){
	...
	return dp[i][j]=min(calculateMinCostOfPairing(i-1,j-1)+abs(bois[i-1]-gurls[j-1]),calculateMinCostOfPairing(i,j-1));
}
void solve()
{
	int n,m;
	...
	cout<<calculateMinCostOfPairing(n,m);
}