...
int dp[105][2][105];
int n,k;
int NumLightArrays(int i,int prev,int cnt){
	if(i==0&&cnt!=k){
		return 0;
	}
	if(i==0&&cnt==k){
	...
	if(i==n){
		int ans=0;
		ans=(ans+NumLightArrays(i-1,0,cnt))%MOD;
		ans=(ans+NumLightArrays(i-1,1,cnt))%MOD;
		return dp[i][prev][cnt]=ans;
	}
	int ans=0;
	if(prev==1){
		ans=(ans+NumLightArrays(i-1,1,cnt+1))%MOD;
	}
	else{
		ans=(ans+NumLightArrays(i-1,1,cnt))%MOD;
	}
