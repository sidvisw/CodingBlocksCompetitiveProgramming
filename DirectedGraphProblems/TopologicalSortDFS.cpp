...
public:
	ll vertices;
	map<string,list<pair<string,ll>>> adjList;
	void addEdge(string u,string v,bool bidir=false,ll w=1)
	{
		adjList[u].pb(mp(v, w));
		if (bidir)
		{
	...
	void dfs(string src, map<string,bool> &visited,list<string>&order)
	{
		visited[src] = true;
		// cout << src << " ";
