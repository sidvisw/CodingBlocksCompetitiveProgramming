...
public:
	...
	void dfs_rev(ll src, vector<bool> &visited)
	{
		visited[src] = true;
		cout << src << " ";
		for (auto v : adjList[src])
		{
			if (!visited[v.ff])
			{
				dfs_rev(v.ff, visited);
			}
		}
	}
	void dfs(ll src, vector<bool> &visited,vi&stack)
	{
		visited[src] = true;
		// cout << src << " ";
		for (auto v : adjList[src])
...
void solve()
{
	...
	vis.resize(n+1);
	char Comp_name='A';
	FORrev(i,(int)stack.size()-1){
		if(!vis[stack[i]]){
			cout<<"Component "<<Comp_name<<" -> ";
			g_rev.dfs_rev(stack[i],vis);
			cout<<endl;
			Comp_name++;
		}
	}
