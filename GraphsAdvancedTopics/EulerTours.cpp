...
public:
	...
	void eulerTour1(ll src,ll parent,vi&tin,vi&tout,int &timer){
		tin[src]=timer++;
		for(auto v:adjList[src]){
			if(v.ff!=parent){
				eulerTour1(v.ff,src,tin,tout,timer);
				tout[src]=timer++;
			}
		}
	}
	void eulerTour2(ll src,ll parent,vi&tin,vi&tout,int &timer){
		tin[src]=timer++;
		for(auto v:adjList[src]){
			if(v.ff!=parent){
				eulerTour2(v.ff,src,tin,tout,timer);
			}
		}
		tout[src]=timer++;
	}
	void eulerTour3(ll src,ll parent,vi&tin,vi&tout,int &timer){
		tin[src]=++timer;
		for(auto v:adjList[src]){
			if(v.ff!=parent){
				eulerTour3(v.ff,src,tin,tout,timer);
			}
		}
		tout[src]=timer;
	}
...
void solve()
{
	...
	vi tin(n+1),tout(n+1);
	cout<<"Euler Tour 1: ";
	g.eulerTour1(1,0,tin,tout,timer);
	cout<<"\nTIME IN: ";
	FORe(i,n){
		cout<<tin[i]<<" ";
	}
	...
	timer=1;
	cout<<"\nEuler Tour 2: ";
	g.eulerTour2(1,0,tin,tout,timer);
	cout<<"\nTIME IN: ";
	FORe(i,n){
		cout<<tin[i]<<" ";
	}
	...
	timer=0;
	cout<<"\nEuler Tour 3: ";
	g.eulerTour3(1,0,tin,tout,timer);
	cout<<"\nTIME IN: ";
	FORe(i,n){
		cout<<tin[i]<<" ";
	}
