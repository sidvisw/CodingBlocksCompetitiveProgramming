...
public:
	...
	void dfs(ll src,ll p, vector<bool> &visited,set<int>&art_p,set<pii>&bridge,vi&discovery,vi&lowest,int &timer)
	{
		visited[src] = true;
		discovery[src]=timer++;
		lowest[src]=discovery[src];
		int n_child=0;
		for (auto v : adjList[src])
		{
			if (!visited[v.ff])
			{
				dfs(v.ff,src, visited,art_p,bridge,discovery,lowest,timer);
				n_child++;
				lowest[src]=min(lowest[src],lowest[v.ff]);
				if(p!=0&&lowest[v.ff]>=discovery[src]){
					art_p.insert(src);
				}
				if(lowest[v.ff]>discovery[src]){
					bridge.insert(mp(src,v.ff));
				}
			}
			else if(v.ff!=p){
				lowest[src]=min(lowest[src],discovery[v.ff]);
			}
		}
		if(p==0&&n_child>1){
			art_p.insert(src);
...
void solve()
{
	...
	set<pii>bridge;
	vector<bool>vis(n+1);
	vi discovery(n+1),lowest(n+1);
	int timer=1;
	g.dfs(1,0,vis,art_p,bridge,discovery,lowest,timer);
