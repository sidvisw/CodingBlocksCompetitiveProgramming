...
public:
	...
	void dfs(ll src, vector<bool> &visited,ll par,vector<ll>&depth,int dep)
	{
		visited[src] = true;
		// cout << src << " ";
		dp[src]=src;
		...
		for (auto v : adjList[src])
		{
			if (!visited[v.ff])
			{
				dfs(v.ff, visited,src,depth,dep+1);
				dp[src]=min(dp[src],dp[v.ff]);
			}
			else if(v.ff!=par){
				dp[src]=min(dp[src],depth[v.ff]);
			}
		}
	}
	ll primsMST()
