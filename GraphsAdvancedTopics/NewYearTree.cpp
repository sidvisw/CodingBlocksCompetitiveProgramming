...
public:
	...
	void dfs(ll src,int parent,int*tin,int*tout,int&timer)
	{
		tin[src]=++timer;
...
/*Your functions here*/
int lazy[4*MAXN+1];
void buildTree(vector<int>&tree,int *c,int s,int e,int index){
	if(s==e){
		tree[index]=1ll<<c[s];
		return;
	}
	int mid=(s+e)/2;
	buildTree(tree,c,s,mid,2*index);
	buildTree(tree,c,mid+1,e,2*index+1);
	tree[index]=tree[2*index]|tree[2*index+1];
}
void update(vector<int>&tree,int s,int e,int l,int r,int c,int index){
	if(lazy[index]!=0){
	...
	int mid=(s+e)/2;
	update(tree,s,mid,l,r,c,2*index);
	update(tree,mid+1,e,l,r,c,2*index+1);
	tree[index]=tree[2*index]|tree[2*index+1];
}
...
void solve()
{
	...
	int timer=-1;
	g.dfs(1,0,tin,tout,timer);
	int mod_c[n];
	FOR(i,n){
		mod_c[tin[i+1]]=c[i];
	}
	vector<int> tree(4*n+1);
	buildTree(tree,mod_c,0,n-1,1);
	while(m--){
		int type;
		cin>>type;
		if(type==1){
			...
			update(tree,0,n-1,l,r,c,1);
		}
		else{
			int node;
			cin>>node;
			int l=tin[node],r=tout[node];
			int q=query(tree,0,n-1,l,r,1);
			cout<<countSetBits(q)<<endl;
		}
	}
}