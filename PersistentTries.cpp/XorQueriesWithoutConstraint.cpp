...
class Trie{
	...
	int lessThanEqualTo(int num){
		...
		for(int i=3;i>=0;i--){
			...
			if(bit&&cur->child[0]!=NULL){
				cnt+=cur->child[0]->count;
			}
			cur=cur->child[bit];
		}
		if(cur!=NULL){
	...
	int kth_smallest(int k){
		...
		int ans=0;
		for(int i=3;i>=0;i--){
			if(cur->child[0]!=NULL&&k<=cur->child[0]->count){
				cur=cur->child[0];
			}
			else{
				if(cur->child[0]!=NULL)k-=cur->child[0]->count;
...
void solve()
{
	...
	FOR(i,m){
		cout<<a[i]<<" "<<t.maxXOR(a[i])<<endl;
	}
	FOR(i,m){
		cout<<a[i]<<" "<<t.lessThanEqualTo(a[i])<<endl;
	}
	FOR(i,m){
		cout<<i+1<<" "<<t.kth_smallest(i+1)<<endl;
	}
}