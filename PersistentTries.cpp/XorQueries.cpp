...
class Trie{
	...
	Trie insert(int num,Trie t){
		Node* curr=root;
		Node* ptr=t.root;
		curr->count=ptr->count+1;
		for(int i=31;i>=0;i--){
			int bit=(num>>i)&1;
			curr->child[bit^1]=ptr->child[bit^1];
...
int maxZor(int num,Trie t1,Trie t2){
	Node *cur1=t1.root;
	Node*cur2=t2.root;
	int ans=0;
	for(int i=31;i>=0;i--){
		int bit=(num>>i)&1;
		if(cur2->child[bit^1]->count-cur1->child[bit^1]->count>=1){
			cur2=cur2->child[bit^1];
			cur1=cur1->child[bit^1];
			ans+=(bit^1)*(1<<i);
		}
		else{
			cur2=cur2->child[bit];
...
int lessThanEqualTo(int num,Trie t1,Trie t2){
	Node* cur1=t1.root;
	Node* cur2=t2.root;
	int ans=0;
	for(int i=31;i>=0;i--){
		int bit=(num>>i)&1;
		if(bit){
			ans+=cur2->child[0]->count-cur1->child[0]->count;
			cur1=cur1->child[1];
			cur2=cur2->child[1];
		}
		else{
	...
	ans+=cur2->count-cur1->count;
	return ans;
}
int kth_element(int k,Trie t1,Trie t2){
	Node*cur1=t1.root;
	Node*cur2=t2.root;
	int ans=0;
	for(int i=31;i>=0;i--){
		if(k<=cur2->child[0]->count-cur1->child[0]->count){
			cur1=cur1->child[0];
			cur2=cur2->child[0];
		}
		else{
			k-=(cur2->child[0]->count-cur1->child[0]->count);
			cur1=cur1->child[1];
			cur2=cur2->child[1];
			ans+=(1<<i);
		}
...
void solve()
{
	...
	while(m--){
		...
		if(type==0){
			...
			cin>>x;
			Trie t;
			v.pb(t.insert(x,v.back()));
		}
		if(type==1){
			int l,r,x;
			cin>>l>>r>>x;
			cout<<maxZor(x,v[l-1],v[r])<<endl;
		}
		if(type==2){
			int k;
			cin>>k;
		...
		if(type==3){
			int l,r,x;
			cin>>l>>r>>x;
			cout<<lessThanEqualTo(x,v[l-1],v[r])<<endl;
		}
		if(type==4){
			int l,r,x;
			cin>>l>>r>>x;
			cout<<kth_element(x,v[l-1],v[r])<<endl;
		}
	}
}