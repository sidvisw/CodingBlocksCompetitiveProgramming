...
public:
	...
	vector<string> getStrings(TrieNode*curr){
		vector<string> ans;
		if(curr->terminal){
			string s;
		...
		for(auto child:curr->children){
			vector<string> childAns = getStrings(child.second);
			for(auto childStr:childAns){
				string s = curr->data + childStr;
				ans.push_back(s);
			}
		}
		return ans;
	...
	vector<string> search(string &s)
	{
		...
		vector<string>res=getStrings(curr);
		for(auto &str:res){
			str=s+str.substr(1);
		}
		return res;
