...
public:
	...
	int maxZor(int num,int l,int r)
	{
		TrieNode *curr = root;
		int ans=0;
		for(int i=31;i>=0;i--){
			int bit=(1<<i)&num;
			if(bit){
				if(curr->left!=NULL&&lower_bound(all(curr->left->indices),l)!=curr->left->indices.end()&&*lower_bound(all(curr->left->indices),l)<=r){
					ans+=(1<<i);
					curr=curr->left;
				}
				else{
				...
				if(curr->right!=NULL&&lower_bound(all(curr->right->indices),l)!=curr->right->indices.end()&&*lower_bound(all(curr->right->indices),l)<=r){
					ans+=(1<<i);
					curr=curr->right;
				}
				else{
