...
/*Your functions here*/
#define int ll
void merge(vi&a,int beg,int mid,int end,int &alphaScore){
    vi temp(end-beg+1);
    int i=beg,j=mid+1,k=0;
    while(i<=mid && j<=end){
        if(a[i]<a[j]){
            temp[k++]=a[i++];
            alphaScore=(alphaScore+(a[i-1]*(end-j+1))%MOD)%MOD;
        }
        else{
            temp[k++]=a[j++];
        }
...
void mergeSort(vi&a,int beg,int end,int&alphaScore){
    if(beg<end){
        int mid=(beg+end)/2;
        mergeSort(a,beg,mid,alphaScore);
        mergeSort(a,mid+1,end,alphaScore);
        merge(a,beg,mid,end,alphaScore);
    }
}
void solve()
{
