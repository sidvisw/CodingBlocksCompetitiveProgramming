...
/*Your functions here*/
int partition(vi&a,int beg,int end){
    int pivot=a[beg];
    int i=beg+1;
    int j=beg+1;
    while(j<=end){
...
int partitionRandom(vi&a,int beg,int end){
    int pivotIdx=rand()%(end-beg+1)+beg;
    swap(a[beg],a[pivotIdx]);
    return partition(a,beg,end);
}
void quickSort(vi&a,int beg,int end){
    if(beg<end){
        int p=partitionRandom(a,beg,end);
        quickSort(a,beg,p-1);
        quickSort(a,p+1,end);
    }
}
