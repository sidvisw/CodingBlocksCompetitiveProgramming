...
/*Your functions here*/
int getMaximumSubarraySum(vi&a,int beg,int end){
    if(beg>end){
        return 0;
    }
    if(beg==end){
    ...
    int mid=(beg+end)/2;
    int subarraySumLeft=getMaximumSubarraySum(a,beg,mid);
    int subarraySumRight=getMaximumSubarraySum(a,mid+1,end);
    int subarraySum=0;
    int leftSum=0;
    for(int i=mid;i>=beg;i--){
        leftSum+=a[i];
        subarraySum=max(subarraySum,leftSum);
    }
    int rightSum=subarraySum;
    for(int i=mid+1;i<=end;i++){
        rightSum+=a[i];
        subarraySum=max(subarraySum,rightSum);
    }
    return max(subarraySum,max(subarraySumLeft,subarraySumRight));
}
void solve()
{
    int n;
    ...
    cout<<getMaximumSubarraySum(a,0,n-1);
}