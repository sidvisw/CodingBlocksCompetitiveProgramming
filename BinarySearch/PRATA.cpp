...
/*Your functions here*/
ll calculatePrata(ll rank,ll time){
    ldd root=sqrt(1+(8.0*time)/rank);
    ll ans=(-1+root)/2.0;
    return ans;
}
bool isPossible(vl&a,ll p,ll time){
    ll totP=0;
    FOR(i,a.size()){
        totP+=calculatePrata(a[i],time);
    }
    if(totP>=p){
        return true;
    }
...
void BinarySearch(vector<ll> &a, ll begin, ll end, ll p, ll &idx)
{
    if (begin > end)
    {
        return;
    ...
    {
        idx = mid;
        BinarySearch(a, begin, mid - 1, p, idx);
    }
    else
    {
        BinarySearch(a, mid + 1, end, p, idx);
    }
}
void solve()
{
    ...
    vl ranks(l);
    FOR(i,l)cin>>ranks[i];
    ll maxTime=p*(p+1)/2*(*min_element(ranks.begin(),ranks.end()));
    ll ans=maxTime;
    BinarySearch(ranks,0,maxTime,p,ans);
    cout<<ans;
}