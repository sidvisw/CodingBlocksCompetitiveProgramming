...
/*Your functions here*/
#define int ll
int calculateOnes(int low,int high,int l,int r,int n){
    if(n==0){
        return 0;
    }
    if(n==1){
    ...
    if(l<=mid-1){
        left=calculateOnes(low,mid-1,l,r,n/2);
    }
    if(r>=mid+1){
        right=calculateOnes(mid+1,high,l,r,n/2);
    }
    if(mid>=l&&mid<=r){
        center=calculateOnes(mid,mid,l,r,n%2);
    }
...
void solve()
{
    ...
    cin >> n >> l >> r;
    l--;r--;
    int maxLength=pow(2,(int)(log2(n)+1))-2;
    cout<<calculateOnes(0,maxLength,l,r,n);
}