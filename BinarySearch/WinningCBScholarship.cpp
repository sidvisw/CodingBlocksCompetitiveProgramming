...
#define int ll
bool isPossible(int lambda,int x,int y,int m,int n){
    if(lambda<=((m+n*y)/(x+y)))return true;
    return false;
}
void BinarySearch(ll begin, ll end, ll &idx, int x,int y,int m,int n)
{
    if (begin > end)
    {
        return;
    ...
    ll mid = (begin + end) / 2;
    if (isPossible(mid,x,y,m,n))
    {
        idx = mid;
        BinarySearch(mid + 1, end, idx,x,y,m,n);
    }
    else
    {
        BinarySearch(begin, mid - 1, idx,x,y,m,n);
    }
}
/*----------------------------------------------------------------------*/
