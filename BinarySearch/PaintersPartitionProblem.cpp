...
bool isPossible(vl&a,int time,int k){
    ll sum=0;
    ll cnt=0;
    FOR(i,a.size()){
        if(sum+a[i]>time){
...
void BinarySearch(vector<ll> &a, ll begin, ll end, ll k, ll &idx)
{
    if (begin > end)
    {
        return;
    ...
    {
        idx=mid;
        BinarySearch(a, begin, mid - 1, k, idx);
    }
    else
    {
        BinarySearch(a, mid + 1, end, k, idx);
    }
}
/*----------------------------------------------------------------------*/
...
void solve()
{
    ...
    BinarySearch(a,*max_element(all(a)),ans,k,ans);
    cout<<ans;
}