...
#define int ll
ll m;
bool isPossible(vector<ll>&a,ll ans){
    ll sum=0;
    ll cnt=0;
    FOR(i,a.size()){
        if(sum+a[i]>ans){
...
void BinarySearch(vector<ll> &a, ll begin, ll end, ll &idx)
{
    if (begin > end)
    {
        return;
    ...
    {
        idx=mid;
        BinarySearch(a, begin, mid - 1, idx);
    }
    else
    {
        BinarySearch(a, mid + 1, end, idx);
    }
}
/*----------------------------------------------------------------------*/
