...
int calculateGrundy(vector<vi>&mat,int x,int y){
	if(mat[x][y]!=-1){
		return mat[x][y];
	}
	int n=mat.size()-1;
	...
	unordered_set<int>s;
	for(auto pos:newPos){
		s.insert(calculateGrundy(mat,pos.ff,pos.ss));
	}
	return mat[x][y]=MEX(s);
}
void solve()
{
	int n;
	cin >> n;
	vector<vi> mat(n+1, vi(n+1,-1));
	int h;
	cin>>h;
	vii horsePos(h);
	FOR(i, h)
