	...
	int dfs(ll src, ll p,ll col)
	{
		...
		for (auto v : adjList[src])
		{
			if (v.ff!=p)
			{
				isLeaf=false;
				if(col){
					ways=(ways*dfs(v.ff,src,col-1))%MOD;
				}
				else{
					ways=(ways*(dfs(v.ff,src,col)+dfs(v.ff,src,col+1))%MOD)%MOD;
				}
			}
		}
		if(isLeaf){
...
void solve()
{
	...
	cout<<(g.dfs(1,0,0)+g.dfs(1,0,1))%MOD;
}