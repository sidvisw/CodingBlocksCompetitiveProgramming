...
/*Your functions here*/
int dp[25][1ll<<21];
int waysToPair(vector<vi>&a,int man,int womanVis){
	if(womanVis==((1ll<<a.size())-1)){
		return 1;
	}
	if(dp[man][womanVis]!=-1){
		return dp[man][womanVis];
	}
	int ans=0;
	for(int i=0;i<a.size();i++){
		if(!(womanVis&(1ll<<i))&&a[man][i]){
			ans=(ans+waysToPair(a,man+1,womanVis|(1ll<<i)))%MOD;
		}
	}
	return dp[man][womanVis]=ans;
}
void solve()
{
	int n;
