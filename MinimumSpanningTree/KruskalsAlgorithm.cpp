...
public:
	...
	ll findSet(ll i,vl&parent){
		if(parent[i]==-1)return i;
		return parent[i]=findSet(parent[i],parent);
	}
	bool unionSet(ll i,ll j,vl&parent,vl&rank){
		ll x=findSet(i,parent);
		ll y=findSet(j,parent);
		if(x!=y){
			if(rank[x]<rank[y]){
				parent[x]=y;
				rank[y]+=rank[x];
	...
	ll KruskalsAlgorithm(){
		priority_queue<pair<ll,pll>,vector<pair<ll,pll>>,greater<pair<ll,pll>>> pq;
		FORe(i,vertices){
			for(auto p:adjList[i]){
				pq.push(mp(p.ss,mp(i,p.ff)));
			}
		}
		vl parent(vertices+1,-1);
		vl rank(vertices+1,1);
		ll ans=0;
		while(!pq.empty()){
			auto p=pq.top();
			pq.pop();
			if(unionSet(p.ss.ff,p.ss.ss,parent,rank)){
				ans+=p.ff;
			}
		}
		return ans;
