...
void buildTree(int*tree,int *dp,int s,int e,int index){
	if(s==e){
		tree[index]=dp[s];
		return;
	}
	int mid=(s+e)/2;
	buildTree(tree,dp,s,mid,2*index);
	buildTree(tree,dp,mid+1,e,2*index+1);
	tree[index]=max(tree[2*index],tree[2*index+1]);
}
void update(int *tree,int s,int e,int l,int r,int val,int index){
	if(lazy[index]!=0){
	...
	int mid=(s+e)/2;
	update(tree,s,mid,l,r,val,2*index);
	update(tree,mid+1,e,l,r,val,2*index+1);
	tree[index]=max(tree[2*index],tree[2*index+1]);
}
int queryLazy(int *tree,int s,int e,int l,int r,int index){
	...
	if(s>=l&&e<=r)
		return tree[index];
	int mid=(s+e)/2;
	int q1=queryLazy(tree,s,mid,l,r,2*index);
	int q2=queryLazy(tree,mid+1,e,l,r,2*index+1);
	return max(q1,q2);
}
void solve()
{
	...
	FOR(i,m){
		cin>>intervals[i].first.ff>>intervals[i].first.ss>>intervals[i].second;
		intervals[i].ff.ff--;
		intervals[i].ff.ss--;
	}
	...
	int dp[n]={0};
	int tree[4*n+1];
	buildTree(tree,dp,0,n-1,1);
	int idx=0;
	while(idx<m&&intervals[idx].ff.ss==0){
		update(tree,0,n-1,intervals[idx].ff.ff,intervals[idx].ff.ss,intervals[idx].second,1);
		idx++;
	}
	FORe(i,n-1){
		update(tree,0,n-1,i,i,max(0ll,queryLazy(tree,0,n-1,0,i-1,1)),1);
		while(idx<m&&intervals[idx].ff.ss==i){
			update(tree,0,n-1,intervals[idx].ff.ff,intervals[idx].ff.ss,intervals[idx].second,1);
			idx++;
		}
	}
	cout<<max(0ll,queryLazy(tree,0,n-1,0,n-1,1));
}