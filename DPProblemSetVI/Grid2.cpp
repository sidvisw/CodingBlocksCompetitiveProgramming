...
/*----------------------------------------------------------------------*/
vi mod_inv;
void mod_inverse_init(int n){
	mod_inv.clear();
	mod_inv.resize(n + 1);
	FOR(i, n+1)
	{
		mod_inv[i] = mod_inverse(factorial[i], MOD);
	}
}
signed main()
{
	...
	factorial_init(200000);
	mod_inverse_init(200000);
	FORe(i, t)
	{
		// cout << "Case #"<<i<<": ";
		solve();
...
/*Your functions here*/
int ncr(int n,int r){
	return (((factorial[n]*mod_inv[r])%MOD)*mod_inv[n-r])%MOD;
}
int pathsBetweenPoints(pii a,pii b){
	if(b.ff>=a.ff && b.ss>=a.ss){
		return ncr(b.ff-a.ff+b.ss-a.ss,b.ss-a.ss);
	}
	return 0;
}
int Ways(int n,int m,vii&obstacles){
	int ways=ncr(n+m-2,n-1);
	vi pathsToObs(obstacles.size());
	for(int i=0;i<obstacles.size();i++){
		pathsToObs[i]=pathsBetweenPoints(pii(1,1),obstacles[i]);
		for(int j=i-1;j>=0;j--){
			pathsToObs[i]=(pathsToObs[i]-(pathsToObs[j]*pathsBetweenPoints(obstacles[j],obstacles[i]))%MOD+MOD)%MOD;
		}
	}
	for(int i=0;i<pathsToObs.size();i++){
		ways=(ways-(pathsToObs[i]*pathsBetweenPoints(obstacles[i],mp(n,m)))%MOD+MOD)%MOD;
	}
	return ways;
}
...
void solve()
{
	...
	vii obstacles(n);
	FOR(i,n){
		cin>>obstacles[i].ff>>obstacles[i].ss;
	}
	sort(all(obstacles),comp);
	cout<<Ways(h,w,obstacles);
}