...
int maxValue(vector<pair<pii,int>>&boxes,int idx,int maxWt){
	if(idx==boxes.size()){
		return 0;
	}
	if(maxWt==INF&&dp[idx][10004]!=-1){
		return dp[idx][10004];
	}
	else if(maxWt!=INF&&dp[idx][maxWt]!=-1){
		return dp[idx][maxWt];
	}
	int ans=0;
	if(boxes[idx].ff.ff<=maxWt){
		ans=max(ans,boxes[idx].ss+maxValue(boxes,idx+1,min(boxes[idx].ff.ss,((maxWt==INF)?INF:maxWt-boxes[idx].ff.ff))));
	}
	ans=max(ans,maxValue(boxes,idx+1,maxWt));
	if(maxWt==INF){
		return dp[idx][10004]=ans;
	}
	return dp[idx][maxWt]=ans;
}
void solve()
{
	int n;
	...
	FOR(i,n)
	{
		int x,y,w;
		cin>>x>>y>>w;
		boxes.pb(mp(mp(x,y),w));
	}
	sort(boxes.begin(),boxes.end(),comp);
	memset(dp,-1,sizeof(dp));
	cout<<maxValue(boxes,0,INF);
}