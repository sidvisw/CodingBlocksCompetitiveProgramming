...
public:
	...
	{
		return ceil((ldd)(L.intercept - intercept) / (slope - L.slope));
	}
	int getX(int x)
	{
...
public:
	...
	void insert(Line L)
	{
		...
		while (!deq.empty() && L.intersect(deq.back().ff) <= deq.back().ss)
		{
			deq.pop_back();
		}
		deq.pb(mp(L, L.intersect(deq.back().ff)));
	}
	int query(int x)
	{
		while (deq.size() > 1 && deq[1].ss <= x)
		{
			deq.pop_front();
		}
		return deq.front().ff.getX(x);
	}
};
void solve()
{
	...
	int dp[n + 1];
	dp[1] = 0;
	dataStruct.insert(Line(-2 * h[1], dp[1] + h[1] * h[1]));
	for (int i = 2; i <= n; i++)
	{
		dp[i] = h[i] * h[i] + c + dataStruct.query(h[i]);
		dataStruct.insert(Line(-2 * h[i], dp[i] + h[i] * h[i]));
	}
	cout << dp[n];
}