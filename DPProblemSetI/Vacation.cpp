...
/*Your functions here*/
int dp[MAXN][3];
int maxPossibleHappiness(vector<vi>&v,int idx,int work){
	if(idx==v.size()){
		return 0;
	}
	if(dp[idx][work]!=-1){
	...
	for(int i=0;i<3;i++){
		if(i!=work)
			ans=max(ans,maxPossibleHappiness(v,idx+1,i));
	}
	return dp[idx][work]=ans+v[idx][work];
}
void solve()
{
	int n;
	...
	cout<<max(maxPossibleHappiness(v,0,0),max(maxPossibleHappiness(v,0,1),maxPossibleHappiness(v,0,2)));
}