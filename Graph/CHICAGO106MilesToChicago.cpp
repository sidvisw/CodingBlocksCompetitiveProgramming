...
public:
	...
	vector<ldd> dijkstra(ll src)
	{
		vector<ldd> dist(vertices + 1, 0);
		set<pair<ldd,ll>,greater<pair<ldd,ll>>> s;
		dist[src] = 100;
		s.insert(mp(100, src));
		while (!s.empty())
		{
			...
			for (auto v : adjList[minEdge.ss])
			{
				if (dist[v.ff] < dist[minEdge.ss] * v.ss/100.0)
				{
					...
					dist[v.ff] = dist[minEdge.ss] * v.ss/100.0;
...
void solve(int n)
{
	...
	vector<ldd> dist=g.dijkstra(1);
	cout<<setprecision(6)<<fixed<<dist[n]<<" percent";
}