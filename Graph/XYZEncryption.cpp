...
void solve()
{
	...
	FORe(i,n){
		FORr(j,i+1,n){
			if(!binary_search(all(edges),mp(i,j))&&!binary_search(all(edges),mp(j,i))){
				uv={i,j};
				break;
			}
		}
	...
	str[uv.ss]='z';
	FORe(i,n){
		if(i==uv.ff||i==uv.ss)continue;
		bool bin1=binary_search(all(edges),mp(i,uv.ff))||binary_search(all(edges),mp(uv.ff,i));
		bool bin2=binary_search(all(edges),mp(i,uv.ss))||binary_search(all(edges),mp(uv.ss,i));
		if(bin1&&bin2){
			str[i]='y';
		}
		else if(bin1){
