...
Euler's Theorem:
	...
	A connected graph has an Euler path if and only if there are two vertices with odd degree.
We need to find Euler Path in this question.
*/
...
public:
	...
	void addEdge(pii u, pii v, bool bidir = true, ll w = 1)
	{
		adjList[u].pb(mp(v, w));
		if (bidir)
		{
	...
	int dfs(pii src,map<pii,bool>&vis){
		vis[src] = true;
		int count=0;
		if(deg[src]&1){
			count++;
...
void solve()
{
	...
	FOR(i, m)
	{
		int x1,y1,x2,y2;
		cin>>x1>>y1>>x2>>y2;
		vertices.insert(mp(x1,y1));
		vertices.insert(mp(x2,y2));
		edges.pb(mp(mp(x1,y1),mp(x2,y2)));
	}
