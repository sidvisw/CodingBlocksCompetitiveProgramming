...
public:
	...
	void dfs(ll src, vector<bool> &visited,list<int>&order)
	{
		visited[src] = true;
		// cout << src << " ";
...
vii Tree;
int n;
void buildTree(vi&a,int s,int e,int index){
	if(s>e)return;
	if(s==e){
		Tree[index]={s,a[s]};
		return;
	...
	if(Tree[2*index].ss<Tree[2*index+1].ss){
...
void makeNodeZero(int s,int e,int idx,int index){
	if(s>e)return;
	...
	int mid=(s+e)/2;
	makeNodeZero(s,mid,idx,2*index);
	makeNodeZero(mid+1,e,idx,2*index+1);
	if(Tree[2*index].ss<Tree[2*index+1].ss){
		Tree[index]=Tree[2*index+1];
	}
	else{
...
pii query(int s,int e,int l,int r,int index){
	if(s>e||s>r||e<l)return mp(-1,-1);
	if(s>=l&&e<=r){
		return Tree[index];
	}
	int mid=(s+e)/2;
	pii lq=query(s,mid,l,r,2*index);
	pii rq=query(mid+1,e,l,r,2*index+1);
	if(lq.ff==-1)return rq;
	if(rq.ff==-1)return lq;
	if(lq.ss<rq.ss)return rq;
	return lq;
}
void dfs(int cur,vector<bool>&vis,list<int>&order){
	makeNodeZero(1,n,cur,1);
	vis[cur]=true;
	if(b[cur]!=n+1&&!vis[b[cur]]){
		dfs(b[cur],vis,order);
	}
	while(1){
		pii bj=query(1,n,1,a[cur]-1,1);
		if(bj.ss>cur){
			if(!vis[bj.ff])
				dfs(bj.ff,vis,order);
		}
