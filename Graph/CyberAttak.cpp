...
bool unionSet(int i,int j,vi&parent,vi&rank){
...
void Kruskal(vector<pair<int,pii>>&edges,vector<pair<int,pii>>&edgesMST,int V)
{
	vi parent(V+1,-1);
	vi rank(V+1,1);
	for(auto edge:edges)
...
void solve()
{
	...
	vector<pair<int,pii>> edgesMST;
	Kruskal(edges,edgesMST,n);
	int sum=0;
	int i;
	F0R(i,edgesMST.size())
	{
