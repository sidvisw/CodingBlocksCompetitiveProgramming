...
void solve()
{
    int n;
    cin>>n;
    vii res=prime_factorization(n);
    if(res.size()>=3){
        cout<<"YES"<<endl;
        cout<<res[0].ff<<" "<<res[1].ff<<" "<<n/(res[0].ff*res[1].ff);
        return;
    }
    if(res.size()==2){
        ll sum=res[0].ss+res[1].ss;
        if(sum>=4){
            cout<<"YES"<<endl;
            cout<<res[0].ff<<" "<<res[1].ff<<" "<<n/(res[0].ff*res[1].ff);
            return;
    ...
    if(res.size()==1){
        if(res[0].ss>=6){
            cout<<"YES"<<endl;
            cout<<res[0].ff<<" "<<res[0].ff*res[0].ff<<" "<<n/(res[0].ff*res[0].ff*res[0].ff);
            return;
        }
    }
    cout<<"NO";
