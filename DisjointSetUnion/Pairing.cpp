...
public:
	...
	ll findSet(int i,vi& parent){
		if(parent[i]==-1)return i;
		return parent[i]=findSet(parent[i],parent);
	}
	void addEdge(ll u, ll v, vi&parent,vi&rank)
	{
		ll x=findSet(u,parent);
		ll y=findSet(v,parent);
		if(x!=y){
			if(rank[x]<rank[y]){
				parent[x]=y;
				rank[y]+=rank[x];
...
void solve()
{
	...
	FOR(i,m){
		int u,v;
		cin>>u>>v;
		g.addEdge(u,v,parent,rank);
	}
	int ans=n*(n-1)/2;
	FOR(i,n){
		if(parent[i]==-1)ans-=rank[i]*(rank[i]-1)/2;
	}
	cout<<ans;
}