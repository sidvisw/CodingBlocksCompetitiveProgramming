...
void unionSet(int i,int j,vi&parent){
...
void solve()
{
	...
	FOR(i,q)
	{
		cin>>queries[i].ff.ff>>queries[i].ff.ss>>queries[i].ss;
	}
	vi parent(n+1,-1);
	FORrev(i,q-1){
		int l=queries[i].ff.ff-1,r=queries[i].ff.ss-1,c=queries[i].ss;
		int idx=findSet(l,parent);
		while(idx<=r){
			v[idx]=c;
