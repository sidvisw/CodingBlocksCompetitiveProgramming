...
vii convex_hull(vii points){
	int n = points.size();
	sort(all(points));
	pii p1=points[0],p2=points[n-1];
	vii up,down;
	up.pb(p1);down.pb(p1);
	for(int i=1;i<n;i++){
		if(!ccw(p1,points[i],p2)){
			while(up.size()>1&&ccw(up[up.size()-2],up[up.size()-1],points[i])){
				up.pop_back();
			}
			up.pb(points[i]);
		}
		if(!cw(p1,points[i],p2)){
			while(down.size()>1&&cw(down[down.size()-2],down[down.size()-1],points[i])){
				down.pop_back();
			}
			down.pb(points[i]);
		}
...
void solve()
{
	...
	vii convexHull=convex_hull(points);
