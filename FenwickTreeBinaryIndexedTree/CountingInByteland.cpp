...
void solve()
{
    ...
    while(q--){
        ...
        if(type==1){
            ...
            x1++;x2++;y1++;y2++;z1++;z2++;
            cout<<query(n+1,n+1,n+1)-query(x2,y2,z2)+query(x1-1,y2,z2)+query(x2,y1-1,z2)+query(x2,y2,z1-1)-query(x1-1,y1-1,z2)-query(x2,y1-1,z1-1)-query(x1-1,y2,z1-1)+query(x1-1,y1-1,z1-1)<<endl;
        }
    }
}