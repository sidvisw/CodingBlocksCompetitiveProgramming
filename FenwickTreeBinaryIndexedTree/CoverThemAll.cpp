...
/*Your functions here*/
#define int ll
void update(vi&BIT,int index,int inc){
	while(index<=BIT.size()-1){
		BIT[index]=(BIT[index]+inc)%MOD;
		index+=index&(-index);
	}
}
int query(vi&BIT,int index){
	int sum=0;
	while(index>0){
...
void solve()
{
	...
	vi BIT(index+1);
	vi bit(index+1);
	update(BIT,m[a[0].ff],a[0].ff);
	update(bit,m[a[0].ff],1);
	int ans=0;
	FORe(i,n-1){
		int aheadSum=(query(BIT,index)-query(BIT,m[a[i].ff])+MOD)%MOD;
		int aheadCnt=(query(bit,index)-query(bit,m[a[i].ff])+MOD)%MOD;
		int behindSum=query(BIT,m[a[i].ff]-1);
		int behindCnt=query(bit,m[a[i].ff]-1);
		ans=(ans+((((aheadSum-(a[i].ff*aheadCnt)%MOD+MOD)%MOD+((a[i].ff*behindCnt)%MOD-behindSum+MOD)%MOD)%MOD)*a[i].ss)%MOD)%MOD;
		update(BIT,m[a[i].ff],a[i].ff);
		update(bit,m[a[i].ff],1);
	}
	cout<<ans;
}