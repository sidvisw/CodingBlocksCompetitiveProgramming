...
/*Your functions here*/
/*
BIT is made up of a hypothrtical cnt[] array which is defined as below-
cnt[i] = (no. of id's such that pre_f[id] = i)
*/
void update(vl&BIT,int index,int inc){
    while(index<=BIT.size()-1){
        BIT[index]+=inc;
        index+=index&(-index);
    }
}
ll query(vl&BIT,int index){
    ll sum=0;
    while(index>0){
        sum+=BIT[index];
        index-=index&(-index);
...
void solve()
{
    ...
    vi a(n);
    vi pre_f(n),suff_f(n);
    map<int,int> pref_freq,suff_freq;
    FOR(i,n){
        cin>>a[i];
        pref_freq[a[i]]++;
        pre_f[i]=pref_freq[a[i]];
    }
    FORrev(i,n-1){
        suff_freq[a[i]]++;
        suff_f[i]=suff_freq[a[i]];
    }
    vl BIT(n+1);
    ll ans=0;
    FOR(i,n){
        ans+=i-query(BIT,suff_f[i]);
        update(BIT,pre_f[i],1);
    }
    cout<<ans;
}