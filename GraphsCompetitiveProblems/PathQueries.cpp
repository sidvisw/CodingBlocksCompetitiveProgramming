...
int unionSet(int i,int j,vi&parent,vi&rank){
...
void solve()
{
	...
	FOR(i, n-1)
	{
		...
		// g.addEdge(edges[i].ss.ff, edges[i].ss.ss, true, edges[i].ff);
	}
	sort(all(edges));
	vi parent(n+1,-1);
	vi rank(n+1,1);
	...
	while(m--){
		int q;
		cin>>q;
		auto it=upper_bound(all(ans),mp(q,INF));
		if(it==ans.begin()){
			cout<<"0 ";
			continue;
		}
