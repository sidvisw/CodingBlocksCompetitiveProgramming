...
void solve()
{
	...
	sort(all(edges));
	int size=min(k,(int)edges.size());
	edges=vector<pair<int,pii>>(edges.begin(),edges.begin()+size);
	set<int>nodes;
	FOR(i,size){
		nodes.insert(edges[i].ss.ff);
		nodes.insert(edges[i].ss.ss);
	}
	n=nodes.size();
	map<int,int>mapping;
	int index=1;
	...
	FOR(i,size){
		...
		int w=edges[i].ff;
		edges[i].ss.ff=mapping[u];
		edges[i].ss.ss=mapping[v];
		adjMat[mapping[u]][mapping[v]]=w;
		adjMat[mapping[v]][mapping[u]]=w;
	}
	FORe(K,n){
		FORe(i,n){
			FORe(j,n){
