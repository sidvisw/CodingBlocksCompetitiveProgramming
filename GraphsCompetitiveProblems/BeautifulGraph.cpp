...
public:
	...
	bool dfs(ll src, vector<bool> &visited,vi&color,int col)
	{
		visited[src] = true;
		color[src]=col;
		if(col==1){
		...
		for (auto v : adjList[src])
		{
			if (!visited[v.ff])
			{
				if(dfs(v.ff, visited,color,3-col))return 1;
			}
			else if(color[v.ff]==color[src]){
				return 1;
			}
		}
		return 0;
...
void solve()
{
	...
	FORe(i,n){
		if(!vis[i]){
			ctr1=0;ctr2=0;
			if(g.dfs(i,vis,color,1)){
				cout<<0;
				return;
			}
			ans=(ans*((fastExponentiation(2,ctr1)+fastExponentiation(2,ctr2))%MOD))%MOD;
		}
	}
	cout<<ans;
}