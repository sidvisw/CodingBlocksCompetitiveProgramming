...
void solve()
{
    ...
    a[0]=0;
    a[1]=1;
    vector<vl> T=generateTransformationMatrix(c);
    ll ans1=0,ans2=0;
    if(m<=1){
        ans1=accumulate(a.begin(),a.begin()+m+1,0ll);
    }
    if(n-1<=1){
        ans2=accumulate(a.begin(),a.begin()+n,0ll);
    }
    ll sum=accumulate(all(a),0ll);
    a.insert(a.begin(),sum);
    if(m>1){
        vector<vl> T1=matrixPower(T,m-1);
        FOR(i,c.size()+1){
            ans1+=(T1[0][i]*a[i])%MOD;
        }
    }
    if(n-1>1){
        vector<vl> T2=matrixPower(T,n-2);
        FOR(i,c.size()+1){
            ans2+=(T2[0][i]*a[i])%MOD;
        }
    }
